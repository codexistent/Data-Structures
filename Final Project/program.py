...
if __name__ == "__main__":
    ...
    # Array size cannot be too small(such as a negative number) or too large
    if not (0 <= n and n <= 999999):
        print("Invalid input.")
        sys.exit()
    ...
    if(algorithm == 0):
        result = algorithm_1(array, k)
    # Algorithm 2 - Merge Sorting - O(N log N)
    elif(algorithm == 1):
        result = algorithm_2(array, k)
    # Algorithm 3 - Hash Table - O(N)
    elif(algorithm == 2):
        result = algorithm_3(array, k)
    # Algorithm 4 - AVL Tree - O(N log N)
    elif(algorithm == 3):
        result = algorithm_4(array, k)
    # Invalid algorithm
    else:
        print("Invalid algorithm.")
        sys.exit()

    # If no good pair is found, 'result' will be None
    if result == None:
        print("No pair found")
    # Otherwise if a good pair is found, 'result' will be equal to the valid pair of numbers that sum up to 'k'
    else:
        print(result)
